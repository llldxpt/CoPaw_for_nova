...
export const STORAGE_PREFIX = "novapaw:message-queue:";
...
function getChannel(): BroadcastChannel | null {
  ...
  try {
    _channel = new BroadcastChannel("novapaw:queue");
  } catch {
    _channel = null;
  }
  return _channel;
...
export const useMessageQueueStore = create<MessageQueueStore>((set, get) => ({
  ...
  enqueue: (sessionId: string, input: QueueItemInput) => {
    ...
    try {
      const agentStorage =
        sessionStorage.getItem("novapaw-agent-storage") ||
        localStorage.getItem("novapaw-agent-storage");
      if (agentStorage) {
