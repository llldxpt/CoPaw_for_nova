...
describe("SidebarSessionList", () => {
  ...
  it("dispatches a DOM event when no click handler is injected", async () => {
    ...
    window.addEventListener("novapaw:sidebar-select-session", listener);
    mockData([sessionA]);
    renderWithProviders(<SidebarSessionList />);
    await waitFor(() => {
      expect(screen.getByTestId("session-item-sess-a")).toBeTruthy();
    ...
    window.removeEventListener("novapaw:sidebar-select-session", listener);
  });
  ...
  it("dispatches a new-chat DOM event when no handler is injected", async () => {
    ...
    window.addEventListener("novapaw:sidebar-new-chat", listener);
    mockData([]);
    renderWithProviders(<SidebarSessionList />);
    const btn = screen
      .getAllByRole("button")
    ...
    window.removeEventListener("novapaw:sidebar-new-chat", listener);
  });
