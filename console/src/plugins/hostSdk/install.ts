/**
 * hostSdk/install.ts — attach `window.NovaPaw.chat`, `window.NovaPaw.host.*`
 * (hooks + fetch), and `window.NovaPaw.audit` to the global namespace.
 *
...
function makeChatNamespace(): NovaPawChatNamespace {
  let anonSeq = 0;
...
export function installHostSdk(): void {
  ...
  const ns = (window.NovaPaw as any) ?? ((window as any).NovaPaw = {});
  ...
  if (!ns.audit) {
    const auditNamespace: NovaPawAuditNamespace = {
      overrides: () => auditStore.overrides(),
    };
    ns.audit = auditNamespace;
  }

  // Extend window.NovaPaw.host with hooks + fetch.
