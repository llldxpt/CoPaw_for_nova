 ...
 * `window.NovaPaw` so plugin bundles can register routes and tool renderers
...
import {
  ...
  buildRouteNamespace,
  buildSlotNamespace,
  type NovaPawAuditNamespace,
  type NovaPawMenuNamespace,
  type NovaPawRouteNamespace,
  type NovaPawSlotNamespace,
} from "./registry/sdk";
...
/** Shared host dependencies exposed to plugin bundles via `window.NovaPaw.host`. */
...
export function installHostExternals(): void {
  ...
  if (!window.NovaPaw) {
    (window as any).NovaPaw = {} as WindowNamespace;
  }

  if (!window.NovaPaw.host) {
    window.NovaPaw.host = {
      React,
      ReactDOM,
      antd,
  ...
  if (!window.NovaPaw.menu) window.NovaPaw.menu = buildMenuNamespace();
  if (!window.NovaPaw.route) window.NovaPaw.route = buildRouteNamespace();
  if (!window.NovaPaw.slot) window.NovaPaw.slot = buildSlotNamespace();
  if (!window.NovaPaw.audit) window.NovaPaw.audit = buildAuditNamespace();
  if (!window.NovaPaw.paw) window.NovaPaw.paw = pawSdkFactory;
  ...
  if (!window.NovaPaw.registerRoutes) {
    window.NovaPaw.registerRoutes = (pluginId, routes) => {
  ...
  if (!window.NovaPaw.registerToolRender) {
    window.NovaPaw.registerToolRender = (pluginId, renderers) => {
