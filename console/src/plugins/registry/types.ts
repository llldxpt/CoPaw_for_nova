/**
 * registry/types.ts — public shapes for the NovaPaw plugin extension registries.
 *
    ...
    dispose() {
      for (const it of d) {
        try {
          ...
          console.warn("[NovaPaw] Disposable threw on dispose:", err);
        }
      }
    },
  };
...
/** A registered route entry (added via builtinRoutes or NovaPaw.route.add). */
