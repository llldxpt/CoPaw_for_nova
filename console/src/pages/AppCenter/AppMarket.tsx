...
export function AppMarket({
  ...
  const [novapawVersion, setNovapawVersion] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    void rootApi
      ...
      .then(({ version }) => setNovapawVersion(version))
      .catch((err) => {
        if (!(err instanceof DOMException && err.name === "AbortError")) {
          setNovapawVersion(null);
        }
      })
      .finally(() => {
  ...
  const requestInstall = useCallback(
    (entry: MarketPluginEntry) => {
      if (installingIdRef.current !== null) return;
      if (isMarketPluginCompatible(entry, novapawVersion)) {
      ...
      Modal.confirm({
        ...
        content: tRef.current("pluginManager.compatWarningContent", {
          ...
          labels: entry.novapaw_compat_labels?.join(", ") ?? "unknown",
