...
vi.mock("@/stores/agentStore", () => {
  const makeState = () => ({
    ...
    agents: [{ id: "default", name: "Default", backend: "novapaw" }],
    setLastChatId: vi.fn(),
...
vi.mock("@/utils/agentBackend", () => ({
  requiresNovaPawModel: vi.fn(() => true),
...
describe("ChatPage coverage", () => {
  ...
  // ── novapaw:open-file-preview event ────────────────────────────────────
  it("handles novapaw:open-file-preview custom event", async () => {
    renderWithProviders(<ChatPage />, {
      initialEntries: ["/chat/test-session"],
    });
    await screen.findByTestId("chat-ui");
    ...
    act(() => {
      window.dispatchEvent(
        new CustomEvent("novapaw:open-file-preview", {
          detail: {
