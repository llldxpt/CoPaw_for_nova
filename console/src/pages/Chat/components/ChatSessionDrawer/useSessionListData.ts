...
/** Sessions from NovaPaw backend include extra fields beyond the runtime UI type */
...
export function useSessionListData(
  ...
  useEffect(() => {
    ...
    window.addEventListener("novapaw:sidebar-switch-done", onDone);
    return () =>
      window.removeEventListener("novapaw:sidebar-switch-done", onDone);
  }, []);
  ...
  const handleDelete = useCallback(
    async (sessionId: string) => {
      ...
      if (currentSessionId) {
        ...
        if (!stillExists) {
          window.dispatchEvent(new CustomEvent("novapaw:sidebar-new-chat"));
        }
      }
    },
