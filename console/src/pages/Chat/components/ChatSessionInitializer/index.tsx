 ...
 *  - novapaw:sidebar-select-session → switch to the given sessionId
 *  - novapaw:sidebar-new-chat       → create a new session
 */
const ChatSessionInitializer: React.FC = () => {
  ...
  useEffect(() => {
          ...
          .finally(() => {
            if (!controller.signal.aborted) {
              ...
              window.dispatchEvent(
                new CustomEvent("novapaw:sidebar-switch-done"),
              );
            }
          });
      }
    ...
    window.addEventListener(
      "novapaw:sidebar-select-session",
      handleSelectSession,
    );
    window.addEventListener("novapaw:sidebar-new-chat", handleNewChat);
    ...
    const pendingNewChat = sessionStorage.getItem("novapaw_pending_new_chat");
    if (pendingNewChat) {
      sessionStorage.removeItem("novapaw_pending_new_chat");
    ...
    return () => {
      ...
      window.removeEventListener("novapaw:sidebar-new-chat", handleNewChat);
    };
