            ...
            input: [
              {
                role: "user",
                metadata: {
                  [NOVAPAW_CLIENT_MESSAGE_ID_KEY]: clientMessageId,
                },
                content: [
                  { type: "text", text: item.text },
                  ...buildAttachmentContentItems(item.attachments),
...
const WIDE_MODE_STORAGE_KEY = "novapaw_chat_wide_mode";
...
function useMultimodalCapabilities(
  ...
  }, [selectedAgent, updateCapsIfChanged, usesNovaPawBackend]);
...
const HISTORY_PANEL_STORAGE_KEY = "novapaw_history_panel_open";
...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("novapaw:open-file-preview", openPreview);
    return () =>
      window.removeEventListener("novapaw:open-file-preview", openPreview);
  }, [dispatchFilesDrawer]);
  ...
  const selectedAgentBackend = selectedAgentInfo?.backend ?? "novapaw";
  const backendCapabilities = selectedAgentInfo?.backend_capabilities;
  const usesNovaPawBackend = requiresNovaPawModel(selectedAgentBackend);
  ...
  }, [selectedAgent, usesNovaPawBackend]);
  ...
  useEffect(() => {
    ...
    sessionApi.onSessionRemoved = (removedId) => {
      ...
      // owned by the delete handlers (via the "novapaw:sidebar-new-chat"
  ...
  const customFetch = useCallback(
    ...
    [extLists, selectedAgent, runningConfigApprovalLevel, usesNovaPawBackend],
  );
  ...
  const handleFileUpload = useCallback(
    async (options: {
      ...
      try {
        ...
        if (usesNovaPawBackend && !multimodalCaps.supportsMultimodal) {
    ...
    [multimodalCaps, t, usesNovaPawBackend],
  );
  ...
  const options = useMemo(() => {
    ...
    const nativeCommands: CommandSuggestion[] = usesNovaPawBackend
      ? [
          {
            command: "/compact",
            value: "compact",
    ...
    // LoopModeSelector; include them in the slash menu when the NovaPaw
    // backend is active. Empty slash_command (default mode) is skipped.
    const loopSuggestions: CommandSuggestion[] = usesNovaPawBackend
    ...
    const handleBeforeSubmit = async () => {
      ...
      if (!isOwnerRef.current) {
        ...
        const queueText = usesNovaPawBackend
      ...
      const textarea = getActiveSenderTextarea();
      if (textarea) {
        const prepared = usesNovaPawBackend
    ...
    const activePluginSuggestions = usesNovaPawBackend ? pluginSuggestions : [];
    ...
    return {
      ...
      welcome: {
        ...i18nConfig.welcome,
        nick: extNick ?? "NovaPaw",
        avatar: extAvatar ?? "/novapaw.png",
      ...
      sender: {
        ...
        actionAffix: (
          ...
          >
            {(usesNovaPawBackend || backendCapabilities?.context_usage) && (
  ...
  return (
    ...
    >
      ...
      >
        ...
        {usesNovaPawBackend && rateLimitAlternatives.length > 0 && (
        ...
        <Modal
          open={usesNovaPawBackend && showModelPrompt}
          closable={false}
          footer={null}
          width={480}
          styles={{
