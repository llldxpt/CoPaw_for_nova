...
export default function AgentsPage() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const active_model =
        values.backend === "novapaw" && providerId && modelId
      ...
      // Mail is only supported for the novapaw backend; never submit
      // mail config for third-party backends (the server rejects it).
      const mail =
        values.backend === "novapaw" &&
      ...
      if (editingAgent) {
        ...
        const result = await agentsApi.createAgent({
          ...
          skill_names: values.backend === "novapaw" ? selectedSkills : [],
        });
