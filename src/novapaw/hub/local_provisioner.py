# -*- coding: utf-8 -*-
"""Fail-closed isolated local process provisioner for NovaPaw Hub."""
...
class LocalProcessRuntimeProvisioner(RuntimeProvisioner):
    """Run a complete NovaPaw process tree inside an OS sandbox."""
    ...
    def __init__(
        ...
        *,
        ...
        self._launcher = ThreadPoolExecutor(
            max_workers=1,
            thread_name_prefix="novapaw-runtime-launcher",
        )
        ...
        try:
            with tempfile.TemporaryDirectory(
                ...
                environment = self.runtime_environment(
                    record,
                    {
                        "NOVAPAW_RUNTIME_INTERNAL_TOKEN": "preflight",
                    },
                )
                try:
    ...
    def start(
        ...
        """Start an isolated local NovaPaw process and wait for readiness."""
        if not credentials.get("NOVAPAW_RUNTIME_INTERNAL_TOKEN"):
            raise RuntimeError(
        ...
        if self._isolator.name == "windows-appcontainer":
            ...
            command = [
                sys.executable,
                "-m",
                "novapaw.hub.windows_runtime_bridge",
                "--control-port",
        ...
        try:
            self._wait_until_ready(
                ...
                isolated.environment.get(
                    "NOVAPAW_RUNTIME_INTERNAL_TOKEN",
                    "",
                ),
            )
    ...
    def runtime_environment(
        ...
        environment["NOVAPAW_WORKING_DIR"] = str(record.working_dir)
        environment["NOVAPAW_SECRET_DIR"] = str(record.secret_dir)
        environment["NOVAPAW_BACKUP_DIR"] = str(record.backup_dir)
        environment[
            "NOVAPAW_KEYRING_ACCOUNT"
        ] = f"novapaw-hub-{record.runtime_id}"
        environment["NOVAPAW_RUNTIME_ID"] = record.runtime_id
        environment["NOVAPAW_TENANT_ID"] = record.tenant_id
        runtime_token = credentials.get("NOVAPAW_RUNTIME_INTERNAL_TOKEN")
        if runtime_token:
            environment["NOVAPAW_RUNTIME_INTERNAL_TOKEN"] = runtime_token
        ...
        while time.monotonic() < deadline:
            ...
            try:
                request = urllib.request.Request(
                    url,
                    headers={
                        "X-NovaPaw-Runtime-Token": runtime_token,
                    },
                )
