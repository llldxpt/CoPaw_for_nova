# -*- coding: utf-8 -*-
"""FastAPI control plane and server entry point for NovaPaw Hub."""
...
def get_hub_root() -> Path:
    ...
    configured = os.environ.get("NOVAPAW_HUB_DIR", "").strip()
    if configured:
...
def build_runtime_service(
    ...
    def runtime_environment(record: Any) -> dict[str, str]:
        ...
        environment[
            ...
            tenant_id=record.tenant_id,
            runtime_id=record.runtime_id,
            name="NOVAPAW_RUNTIME_INTERNAL_TOKEN",
        )
        return environment
...
def create_hub_app(  # pylint: disable=too-many-statements
    ...
    app = FastAPI(title="NovaPaw Hub", lifespan=lifespan)
    ...
    async def ensure_personal_runtime(user: HubUser) -> RuntimeRecord:
        ...
        if record.state is not RuntimeState.RUNNING:
            try:
                ...
                raise HTTPException(
                    status_code=503,
                    detail=f"Personal NovaPaw failed to start: {exc}",
                ) from exc
        return record
    ...
    async def restart_own_runtime(
        ...
        try:
            ...
            raise HTTPException(
                status_code=503,
                detail=f"Personal NovaPaw failed to restart: {exc}",
            ) from exc
        await record_audit(
            user,
            "runtime.restart",
        ...
        try:
            async with httpx.AsyncClient(
                ...
                upstream = await client.get(
                    target,
                    headers={
                        "X-NovaPaw-Runtime-Token": internal_token,
                    },
                )
        except httpx.HTTPError as exc:
            raise HTTPException(
                status_code=502,
                detail=f"Personal NovaPaw is unavailable: {exc}",
            ) from exc
        excluded_headers = {
            "connection",
            "content-length",
    ...
    async def personal_runtime_proxy(
        ...
        headers["X-NovaPaw-Runtime-Token"] = internal_token
    ...
    async def hub_console(path: str) -> Response:
        ...
        return JSONResponse(
            {
                "message": (
                    "NovaPaw Hub is running, but Console assets are "
                    "unavailable. "
...
def run_hub_app(
    *,
    ...
    """Run the NovaPaw Hub control plane with safe public-bind defaults."""
    public_bind = not is_loopback_host(host)
    if public_bind and not force_public:
        raise ValueError(
            "NovaPaw Hub refuses a non-loopback host by default. "
    ...
    if public_bind:
        ...
        warning = (
            "NovaPaw Hub is accepting network connections at "
