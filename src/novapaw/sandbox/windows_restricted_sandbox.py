...
# Sandbox user group
SANDBOX_USERS_GROUP = "NovapawUsers"
...
def _provision_sandbox_user(username: str, password: str) -> bool:
    ...
    _ensure_local_group(
        SANDBOX_USERS_GROUP,
        "NovaPaw sandbox internal group (managed)",
    )
...
def _ensure_python_dir_group_acl() -> None:
    """Grants RX to the NovapawUsers group on the Python install directory.
    ...
    marker_path = os.path.join(python_dir, ".novapaw_acl_granted")
...
def _remove_python_dir_acl_marker() -> None:
    """Removes the .novapaw_acl_granted marker and resets the flag.

    Targets the Python install directory's marker file.

    Called during shutdown_cleanup so that the next sandbox creation will
    re-grant the ACL to the (re-created) NovapawUsers group. Without this,
...
def _install_wfp_block_filters(username: str, user_sid: str) -> bool:
    ...
    rule_name_out = f"NovaPaw_Block_{username}_Out"
    rule_name_in = f"NovaPaw_Block_{username}_In"
...
_sandbox_state_dir = (
    Path(os.environ.get("USERPROFILE", os.path.expanduser("~"))) / ".novapaw"
)
...
def _create_new_sandbox(
    ...
    sandbox_id = f"novapaw_{fingerprint[:12]}"
    username = sandbox_id
    password = _random_password()
    ...
    # machine-name suffix (e.g. "novapaw_xxx.DESKTOP-E7LJ27U").
...
async def _acquire_sandbox(config: SandboxConfig) -> _SandboxInstance:
    ...
    sandbox_name = f"novapaw_{fingerprint[:12]}"
...
def shutdown_cleanup() -> None:
    ...
    by other still-running NovaPaw processes are left untouched.
