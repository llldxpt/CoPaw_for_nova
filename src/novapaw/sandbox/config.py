...
This is the natural entry point into the :mod:`novapaw.sandbox` package:
...
Backend layout (``create_sandbox`` dispatches to these by ``SandboxMode``):
  - SEATBELT     → mod:`novapaw.sandbox.macos_sandbox`      (MacOSSandbox)
  - BUBBLEWRAP   → mod:`novapaw.sandbox.bubblewrap_sandbox` (BubblewrapSandbox)
  - LANDLOCK     → mod:`novapaw.sandbox.linux_sandbox`      (LinuxSandbox)
  - APPCONTAINER → Windows sandbox, dispatched on ``allow_read_all``:
      True  → windows_restricted_sandbox (WindowsRestrictedSandbox)
      False → mod:`novapaw.sandbox.windows_sandbox`            (WindowsSandbox)
  - NONE         → mod:`novapaw.sandbox.local_sandbox`      (NoneSandbox)

Shared base class for all backends:
  - :class:`novapaw.sandbox.local_sandbox.LocalSandbox`
