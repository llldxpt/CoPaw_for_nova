...
def _get_env(key: str, default: str = "") -> str:
    """Look up an env var with automatic NOVAPAW_ legacy fallback.

    Primary key is always used as-is.  When the primary key starts with
    ``NOVAPAW_``, the corresponding ``NOVAPAW_`` variant is transparently
    checked as a fallback so that existing deployments keep working.
    """
    if key in os.environ:
        return os.environ[key]
    if key.startswith("NOVAPAW_"):
        legacy_key = "NOVAPAW_" + key[len("NOVAPAW_") :]
...
class EnvVarLoader:
    ...
    and defaults.  Pass NOVAPAW_* keys; NOVAPAW_* legacy variants are
...
CUSTOM_AGENT_STARTUP_CONCURRENCY_ENV = (
    "NOVAPAW_CUSTOM_AGENT_STARTUP_CONCURRENCY"
)
...
# 1. NOVAPAW_WORKING_DIR / NOVAPAW_WORKING_DIR env var is set → use it
# 2. ~/.novapaw exists (legacy installation) → use it as-is
# 3. Default → ~/.novapaw
_explicit_working_dir = _get_env("NOVAPAW_WORKING_DIR")
if _explicit_working_dir:
    WORKING_DIR = Path(_explicit_working_dir).expanduser().resolve()
else:
    _legacy_novapaw_dir = Path("~/.novapaw").expanduser()
    if _legacy_novapaw_dir.exists():
        WORKING_DIR = _legacy_novapaw_dir.resolve()
    else:
        WORKING_DIR = Path("~/.novapaw").expanduser().resolve()
# Load user-level .env (e.g. ~/.novapaw/.env) after WORKING_DIR is resolved.
...
KEYRING_ACCOUNT_ENV = "NOVAPAW_KEYRING_ACCOUNT"

PROJECT_NAME = "NovaPaw"

# Message metadata tags shared across agent middleware and memory managers.
NOVAPAW_MESSAGE_TAG_KEY = "novapaw_tag"
NOVAPAW_CLIENT_MESSAGE_ID_KEY = "novapaw_client_message_id"
...
def _resolve_docs_dir() -> Path | None:
    """Find NovaPaw documentation directory across all install methods."""
...
JOBS_FILE = EnvVarLoader.get_str("NOVAPAW_JOBS_FILE", "jobs.json")

CHATS_FILE = EnvVarLoader.get_str("NOVAPAW_CHATS_FILE", "chats.json")


# Builtin Q&A helper profile.  agent_id keeps "NovaPaw" prefix for existing
...
# NovaPaw-era builtin QA; may remain in config.json — disabled when the current
# ``BUILTIN_QA_AGENT_ID`` profile is first created (see ``migration``), not
# every startup, so users can re-enable this id if they want.
LEGACY_QA_AGENT_ID = "NovaPaw_QA_Agent_0.1beta1"

TOKEN_USAGE_FILE = EnvVarLoader.get_str(
    "NOVAPAW_TOKEN_USAGE_FILE",
    "token_usage.json",
)

CONFIG_FILE = EnvVarLoader.get_str("NOVAPAW_CONFIG_FILE", "config.json")

HEARTBEAT_FILE = EnvVarLoader.get_str("NOVAPAW_HEARTBEAT_FILE", "HEARTBEAT.md")
...
DOCS_ENABLED = EnvVarLoader.get_bool("NOVAPAW_OPENAPI_DOCS", False)
...
# Plugin directory (installed via `novapaw plugin install`)
...
MEMORY_COMPACT_KEEP_RECENT = EnvVarLoader.get_int(
    "NOVAPAW_MEMORY_COMPACT_KEEP_RECENT",
    3,
    min_value=0,
)
...
# Example: NOVAPAW_CORS_ORIGINS="http://localhost:5173,http://127.0.0.1:5173"
# When unset, CORS middleware is not applied.
CORS_ORIGINS = EnvVarLoader.get_str("NOVAPAW_CORS_ORIGINS", "").strip()

# Upload size limit (MB).  None = no limit.
UPLOAD_MAX_SIZE_MB: int | None = (
    int(v)
    if (v := EnvVarLoader.get_str("NOVAPAW_UPLOAD_MAX_SIZE_MB", ""))
    .strip()
    .isdigit()
    else None
)
...
LLM_STREAM_FIRST_CONTENT_TIMEOUT = EnvVarLoader.get_float(
    "NOVAPAW_LLM_STREAM_FIRST_CONTENT_TIMEOUT",
    30.0,
    min_value=0.0,
)
...
LLM_STREAM_IDLE_TIMEOUT = EnvVarLoader.get_float(
    "NOVAPAW_LLM_STREAM_IDLE_TIMEOUT",
    30.0,
    min_value=0.0,
)
...
try:
    TOOL_GUARD_APPROVAL_TIMEOUT_SECONDS = max(
        float(
            _get_env("NOVAPAW_TOOL_GUARD_APPROVAL_TIMEOUT_SECONDS", "300"),
        ),
        1.0,
    )
except (TypeError, ValueError):
...
try:
    TOOL_GUARD_APPROVAL_HEARTBEAT_INTERVAL = max(
        float(
            _get_env("NOVAPAW_TOOL_GUARD_APPROVAL_HEARTBEAT_INTERVAL", "15"),
        ),
        5.0,
    )
except (TypeError, ValueError):
...
try:
    CAPABILITY_CACHE_TTL_SECONDS = max(
        float(
            _get_env("NOVAPAW_CAPABILITY_CACHE_TTL_SECONDS", "1800"),
        ),
        0.0,
    )
except (TypeError, ValueError):
