...
def _desktop_python() -> Optional[str]:
    ...
    path = os.environ.get("NOVAPAW_DESKTOP_PY_RUNTIME", "").strip()
...
def _ensure_plugin_site_on_path() -> None:
    ...
    os.environ["NOVAPAW_PLUGIN_SITE"] = site_dir
...
class PluginLoader:
    ...
    def _check_version_compatibility(
        ...
        """Check plugin compatibility with current NovaPaw version.

        Uses left-closed, right-open semantics: ``>=min, <max``.
        When ``novapaw_version`` is absent, falls back to legacy
    ...
    async def _load_backend_module(
        ...
        try:
            ...
            if manifest.novapaw_version is not None:
                qv_dict = manifest.novapaw_version.model_dump()
            else:
                qv_dict = {
    ...
    def _install_requirements(
        ...
        the case for uv-managed venvs created by the NovaPaw script
    ...
    def _install_requirements_frozen(
        ...
        if python is None:
            raise RuntimeError(
                ...
                "(NOVAPAW_DESKTOP_PY_RUNTIME not set). Reinstall NovaPaw "
    ...
    async def unload_plugin(
        ...
        removes the plugin's tools from ``novapaw.agents.tools``.
    ...
    def _cleanup_plugin_tools(
        ...
        """Remove plugin tools from ``novapaw.agents.tools``.
        ...
        """
        try:
            tools_module = sys.modules.get("novapaw.agents.tools")
