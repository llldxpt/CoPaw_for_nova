...
_ENC_PREFIX = "ENC:"
_KEYRING_SERVICE = "novapaw"
_KEYRING_SERVICE_LEGACY = "novapaw"
...
def _keyring_account() -> str:
    ...
    via ``NOVAPAW_WORKING_DIR``) would therefore share — and silently
    overwrite — the stable install's master key, leaving the stable install
    unable to decrypt its own secrets.

    Resolution:

    1. Explicit ``NOVAPAW_KEYRING_ACCOUNT`` override always wins (useful for
    ...
    relocated = bool(
        EnvVarLoader.get_str("NOVAPAW_WORKING_DIR")
        or EnvVarLoader.get_str("NOVAPAW_SECRET_DIR"),
    )
    if not relocated:
...
def _should_skip_keyring() -> bool:
    ...
    if EnvVarLoader.get_bool("NOVAPAW_DISABLE_KEYRING"):
        return True

    if EnvVarLoader.get_bool("NOVAPAW_RUNNING_IN_CONTAINER"):
        return True
...
def _try_keyring_get() -> Optional[str]:
    ...
    try:
        ...
        def _get():
            ...
            if value:
                return value
            # Backward compatibility: read legacy NovaPaw keyring entry.
