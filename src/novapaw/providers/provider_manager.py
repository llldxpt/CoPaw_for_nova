...
from novapaw.exceptions import ModelNotFoundException
...
class ProviderManager:  # pylint: disable=too-many-public-methods
    ...
    def _normalize_provider_id(provider_id: str) -> str:
        ...
        Maps legacy 'copaw-local' to 'novapaw-local'.
        """
        if provider_id == "copaw-local":
            return "novapaw-local"
        return provider_id
    ...
    def start_local_model_resume(self, local_manager) -> None:
        ...
        task = asyncio.create_task(
            ...
            name="novapaw-local-model-resume",
        )
    ...
    def _migrate_novapaw_config(self) -> None:
        """Migrate copaw-local provider config to novapaw-local."""
        # 1. Migrate active model configuration (only provider_id)
        if (
            self.active_model
            and self.active_model.provider_id == "copaw-local"
        ):
            self.active_model.provider_id = "novapaw-local"
            self.save_active_model(self.active_model)
            logger.info(
                "Migrated active model provider from "
                "'copaw-local' to 'novapaw-local'",
            )

        # 2. Migrate stored provider config file
        novapaw_config_path = self.builtin_path / "copaw-local.json"
        if not novapaw_config_path.exists():
            return

        try:
            # Load old config and apply to new provider instance
            with open(novapaw_config_path, "r", encoding="utf-8") as f:
                old_config = json.load(f)

            # Get the new built-in provider instance
            provider = self.builtin_providers.get("novapaw-local")
            if not provider:
                return
            ...
            novapaw_config_path.unlink()
            logger.info(
                "Migrated provider config from "
                "'copaw-local.json' to 'novapaw-local.json'",
            )
            ...
            if active_model:
                try:
                    ...
                    if active_model.get("provider_id") == "copaw-local":
                        active_model["provider_id"] = "novapaw-local"
    ...
    def _init_from_storage(self):
        ...
        # Migrate copaw-local to novapaw-local for backwards compatibility
        self._migrate_novapaw_config()
