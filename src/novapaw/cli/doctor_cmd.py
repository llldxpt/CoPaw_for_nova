# -*- coding: utf-8 -*-
"""`novapaw doctor` — read-only checks.

`novapaw doctor fix` — conservative repairs with backup.
"""
...
def _check_api_health(
    ...
    try:
        ...
        click.echo(
            f"Hint: start the server with `novapaw app` (default {base}).",
            err=True,
        )
        return False, None
...
def _doctor_server_python_mismatch_note(
    ...
    if server_exe and doctor_exe:
        if not _same_python_executable(doctor_exe, server_exe):
            return (
                "This `novapaw doctor` is not using the same Python "
                "executable as the running `novapaw app` — diagnostics and "
    ...
    if doctor_env.strip() != server_env.strip():
        return (
            ...
            f"`novapaw app` (doctor: {doctor_env!r}; server: "
            f"{server_env!r}). "
...
def _check_web_auth(base: str) -> tuple[bool, str]:
    ...
    if not has_registered_users():
        return (
            ...
            f"        1) Start `novapaw app`, open {base}/ in a browser.\n"
            "        2) Complete registration (single user) on the login "
            "page.\n"
            "        For automation, set NOVAPAW_AUTH_USERNAME and "
            "NOVAPAW_AUTH_PASSWORD (legacy NOVAPAW_* names still work) — the "
...
async def _check_active_llm(
    ...
    if (
        ...
        return (
            False,
            "no active LLM slot — run `novapaw models list` and configure "
            "an active model",
            [],
        )
    ...
    if deep and pid in ("novapaw-local", "copaw-local"):
        deep_notes = novapaw_local_llm_deep_notes()
...
def run_doctor_checks(
    ...
    """Run read-only ``novapaw doctor`` checks (no disk mutations)."""
    ...
    if config_ok:
        ...
        _doctor_fix_hint(
            ...
            "`novapaw doctor fix --dry-run --help` and `--only`.",
        )
    ...
    if raw_cfg is not None:
        ...
        if unknown:
            ...
            _doctor_fix_hint(
                ...
                "(`novapaw doctor` and `doctor fix` do not strip unknown keys "
                "yet).",
            )
    ...
    if config_ok:
        ...
        if ws_ok:
            ...
            _doctor_fix_hint(
                "Preview the plan (no writes): `novapaw doctor fix --dry-run "
        ...
        if not ext_nonempty:
            click.echo(
                click.style("OK", fg="green")
                + " — no extension notes (register via novapaw.doctor entry "
                "points or register_doctor_contribution; legacy "
                "novapaw.doctor is still loaded)",
            )
        else:
        ...
        click.echo(
            ...
            + _skipped_when_cfg_invalid
            + ". Fix the config file, then re-run `novapaw doctor`.",
        )
        click.echo("\n=== Browser (browser_use / Playwright) ===")
    ...
    if wd_ok:
        ...
        _doctor_fix_hint(
            "Fix: set `NOVAPAW_WORKING_DIR` (or legacy `NOVAPAW_WORKING_DIR`) "
            "or run `novapaw init`. "
            "Preview the plan (no writes): `novapaw doctor fix --dry-run "
    ...
    if llm_ok:
        ...
        _doctor_fix_hint(
            "`novapaw models list` / console model settings — not a "
            "filesystem fix.",
        )
...
@click.option(
    ...
    help=(
        ...
        "uses --timeout) and, when the active model is novapaw-local, "
