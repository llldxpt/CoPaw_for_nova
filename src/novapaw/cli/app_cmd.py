...
def _warn_if_auth_off_non_loopback_bind(host: str, port: int) -> None:
    """Warn when NovaPaw is reachable beyond loopback without auth."""
    ...
    warning = f"""
============================================================
SECURITY NOTICE: NovaPaw is bound to {bind_address} without authentication.

Anyone who can reach this address may access NovaPaw APIs without login.

Recommended:
  - Restrict access to a trusted network interface or protected environment.
  - Enable authentication with NOVAPAW_AUTH_ENABLED=true if untrusted users or
...
def app_cmd(
    ...
    if workers is not None:
        ...
        click.echo(
            "   NovaPaw always uses 1 worker for stability. "
    ...
    if reload:
        os.environ["NOVAPAW_RELOAD_MODE"] = "1"
    else:
