...
from novapaw.plugins.validation import (
...
def _get_api_base() -> Optional[str]:
    """Return the base URL of the running NovaPaw API, or None.
...
def _api_install_plugin(source: str, force: bool = False) -> bool:
    """Send a hot-install request to the running NovaPaw API.
...
def _api_upload_plugin(zip_path: Path, force: bool = False) -> bool:
    """Send a ZIP file to the running NovaPaw API for hot-install.
...
def _api_uninstall_plugin(plugin_id: str) -> bool:
    """Send a hot-uninstall request to the running NovaPaw API.
...
def _is_running() -> bool:
    """Return whether NovaPaw is currently running.

    Returns:
        ``True`` if the API is reachable, ``False`` otherwise.
    """
    from ..config.utils import is_novapaw_running

    return is_novapaw_running()
...
def install(source: str, force: bool):
    ...
    When NovaPaw is running, the plugin is hot-loaded immediately via
    the API (no restart required).  When NovaPaw is stopped, the
    plugin files are copied and will be loaded on next start.

    Examples:
        novapaw plugin install examples/plugins/idealab-provider
        novapaw plugin install /path/to/plugin
        novapaw plugin install https://example.com/plugin.zip
    """
    # If the app is running, delegate to the live API for hot-install
    if _is_running():
        click.echo(
            "NovaPaw is running — using hot-install via API...",
        )
    ...
    click.echo("\nNext steps:")
    click.echo("   1. Start NovaPaw to load the plugin")
...
def uninstall(plugin_id: str):
    ...
    When NovaPaw is running, the plugin is unloaded immediately via
    the API (no restart required).  When NovaPaw is stopped, only the
    ...
    if _is_running():
        click.echo(
            "NovaPaw is running — using hot-uninstall via API...",
        )
