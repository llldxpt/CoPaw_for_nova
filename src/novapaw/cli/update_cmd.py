...
class InstallInfo:
    """Information about the current NovaPaw installation."""
...
class RunningServiceInfo:
    """Detected NovaPaw service endpoint state."""
...
def _fetch_pypi_release_data() -> dict[str, Any]:
    """Fetch novapaw release metadata from PyPI."""
    try:
        resp = httpx.get(
            _PYPI_JSON_URL,
            timeout=10.0,
        ...
        raise click.ClickException(
            f"Failed to fetch the latest NovaPaw version from PyPI: {exc}",
        ) from exc
    except json.JSONDecodeError as exc:
        raise click.ClickException(
            "Received an invalid response from PyPI when checking for the "
            f"latest NovaPaw version: {exc}",
        ) from exc
    if not isinstance(data, dict):
        raise click.ClickException(
            "Received an invalid response from PyPI when checking for the "
            "latest NovaPaw version.",
        )
    return data
...
def _select_latest_version(
    ...
    *,
    ...
    if include_prerelease:
        ...
        if not version:
            raise click.ClickException(
                "Unable to determine the latest NovaPaw version.",
            )
        return version
    ...
    if not candidates:
        raise click.ClickException(
            "Unable to determine the latest stable NovaPaw version.",
        )
    return str(max(candidates))


def _fetch_latest_version(*, include_prerelease: bool = False) -> str:
    """Fetch the latest published NovaPaw version from PyPI."""
...
def _detect_installation() -> InstallInfo:
    ...
    dist = metadata.distribution("novapaw")
...
def _probe_service(base_url: str) -> RunningServiceInfo:
    """Probe a possible running NovaPaw HTTP service."""
    try:
        resp = httpx.get(
...
def _process_candidate_ports() -> list[int]:
    """Infer candidate local NovaPaw service ports from running processes."""
    ports: list[int] = []
    for _pid, command in _process_table():
        if not _is_novapaw_service_command(command):
            continue
...
def _detect_running_service(
    ...
    """Detect whether a NovaPaw HTTP service is currently running."""
    candidates: list[str] = []
    seen: set[str] = set()
    preferred_hosts: list[str] = []
...
def _running_service_display(running: RunningServiceInfo) -> str:
    ...
    if not running.base_url:
        return "a running NovaPaw service"
    version_suffix = f" (version {running.version})" if running.version else ""
    return f"NovaPaw service at {running.base_url}{version_suffix}"


def _confirm_force_shutdown(running: RunningServiceInfo) -> bool:
    """Ask whether `novapaw shutdown` should be used before updating."""
    click.echo("")
    click.secho("!" * 72, fg="yellow", bold=True)
    click.secho(
        "WARNING: RUNNING NOVAPAW SERVICE DETECTED",
        fg="yellow",
        bold=True,
    )
    click.secho("!" * 72, fg="yellow", bold=True)
    ...
    click.secho(
        "Running `novapaw shutdown` will forcibly terminate the current "
        "NovaPaw backend/frontend processes.",
        fg="red",
        bold=True,
    )
    click.secho(
    ...
    return click.confirm(
        "Run `novapaw shutdown` now and continue with the update?",
        default=False,
    )
...
def _run_shutdown_for_update(
    ...
    """Run `novapaw shutdown` in the current environment before updating."""
    command = [info.python_executable, "-m", "novapaw"]
    ...
    click.echo("")
    click.echo("Running `novapaw shutdown` before updating...")
    ...
    try:
        ...
        raise click.ClickException(
            "Failed to run `novapaw shutdown`: " f"{exc}",
        ) from exc
    ...
    if result.returncode != 0:
        raise click.ClickException(
            "`novapaw shutdown` failed. Please stop the running NovaPaw "
            "service manually before running `novapaw update`.",
        )
...
def _build_upgrade_command(
    ...
    *,
    ...
    package_spec = f"novapaw=={latest_version}"
...
def _spawn_update_worker(
    ...
    *,
    ...
    worker_code = (
        "from novapaw.cli.update_cmd import run_update_worker; "
        "import sys; "
...
def _run_update_worker_foreground(plan_path: Path) -> int:
    ...
    try:
        ...
        click.echo("")
        click.echo("[novapaw] Update interrupted. Stopping installer...")
...
def run_update_worker(plan_path: str | Path) -> int:
    ...
    click.echo("")
    click.echo(
        "[novapaw] Updating NovaPaw "
        f"{plan['current_version']} -> {plan['latest_version']}...",
    )
    click.echo(f"[novapaw] Using installer: {plan['installer_label']}")
    ...
    try:
        ...
        click.echo(f"[novapaw] Update failed: {exc}")
        return_code = 1
    finally:
        try:
    ...
    if return_code == 0:
        click.echo("[novapaw] Update completed successfully.")
        click.echo(
            "[novapaw] Please restart any running NovaPaw service "
            "to use the new version.",
        )
    else:
        click.echo(f"[novapaw] Update failed with exit code {return_code}.")
        click.echo(
            "[novapaw] Please fix the error above and run "
            "`novapaw update` again.",
        )
...
def update_cmd(ctx: click.Context, yes: bool, prerelease: bool) -> None:
    """Upgrade NovaPaw in the current Python environment."""
    ...
    if version_check is False:
        click.echo("NovaPaw is already up to date.")
        return
    ...
    if running.is_running:
        if yes:
            raise click.ClickException(
                ...
                "Please stop it before running `novapaw update`, or rerun "
                "without `--yes` to confirm a forced `novapaw shutdown`.",
            )
        ...
        if running.is_running:
            raise click.ClickException(
                f"Detected {_running_service_display(running)} "
                "after `novapaw shutdown`. "
                "Please stop it manually before running `novapaw update`.",
            )

    if not yes and not click.confirm(
        f"Update NovaPaw to {latest_version} in the current environment?",
        default=True,
    ):
        click.echo("Cancelled.")
        return
