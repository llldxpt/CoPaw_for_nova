...
def _submit_background_task(
    ...
    try:
        ...
        click.echo(f"  novapaw agents chat --background --task-id {task_id}")
...
def _check_task_status(
    ...
    try:
        ...
        if status == "finished":
            ...
            click.echo(
                f"  novapaw agents chat --background --task-id {task_id}",
            )
...
def agents_group() -> None:
    ...
    \b
    Examples:
      novapaw agents chat --from-agent bot_a --to-agent bot_b --text "..."
      novapaw agents create --name "Research Bot" --agent-id research_bot
      novapaw agents delete research_bot
      novapaw agents list
    """
...
def list_agents(ctx: click.Context, base_url: Optional[str]) -> None:
    ...
    Examples:
      novapaw agents list
      novapaw agents list --base-url http://192.168.1.100:8088
...
def delete_cmd(
    ...
    \b
    AGENT_ID  Configured agent ID, obtainable via `novapaw agents list`.

    \b
    Examples:
      novapaw agents delete research
      novapaw agents delete research --remove-workspace
      novapaw agents delete research --yes
    """
...
def chat_cmd(
    ...
    Background Task Mode (NEW):
      # Submit complex task
      novapaw agents chat --background \\
      ...
      novapaw agents chat --background --task-id <task_id>
    ...
    \b
    Prerequisites:
      1. Use 'novapaw agents list' to discover available agents
      2. Ensure target agent (--to-agent) is configured and running
      3. Use 'novapaw chats list' to find existing sessions (optional)
