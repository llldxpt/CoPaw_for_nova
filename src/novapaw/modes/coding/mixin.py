...
The internal NovaPaw workspace (configs, sessions, memory) is at:
...
class CodingModeMixin:
    ...
    At runtime this class is mixed into ``NovaPawAgent`` and combined
    with ``Agent`` via MRO. Coding Mode prompt injection is handled by
    :class:`~novapaw.runtime.prompt_contributors.CodingModeContributor`.
    """
    ...
    # ------------------------------------------------------------------
    # Tool registration hook (called from NovaPawAgent._create_toolkit)
    # ------------------------------------------------------------------
