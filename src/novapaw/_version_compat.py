# -*- coding: utf-8 -*-
"""Plugin–NovaPaw version compatibility check.
...
def check_plugin_version_compat(
    ...
    """Check whether a plugin is compatible with the running NovaPaw.
    ...
    if current < min_v or current >= max_v:
        msg = f"requires NovaPaw >={min_v}, <{max_v}, current is {current}"
        return False, msg
    return True, ""
