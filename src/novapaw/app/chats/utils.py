...
from novapaw.agents.context.scroll.serialize import strip_headline
...
def _is_scroll_memory_placeholder(msg: Msg) -> bool:
    ...
    if (
        ...
        and metadata.get(NOVAPAW_MESSAGE_TAG_KEY) == SCROLL_MEMORY_MESSAGE_TAG
    ):
        return True
...
def _is_synthetic_user_message(msg: Msg) -> bool:
    ...
    return (
        isinstance(metadata, dict)
        and metadata.get(NOVAPAW_MESSAGE_TAG_KEY)
...
def build_env_context(
    ...
    parts.append(
        ...
        f"You operate in NovaPaw, an open-source agent "
