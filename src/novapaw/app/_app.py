...
async def lifespan(  # pylint: disable=too-many-statements,too-many-branches
    ...
    try:
        ...
        message = (
            "NovaPaw startup failed because restore artifact cleanup did not "
    ...
    async def _background_startup():  # pylint: disable=too-many-statements
        try:
            ...
            asyncio.create_task(
                ...
                name="novapaw-provider-model-sync",
            )
            asyncio.create_task(
                provider_manager.sync_remote_catalogs(),
                name="novapaw-provider-catalog-sync",
            )
            ...
            from novapaw.runtime.commands.control import register_command
...
_CONSOLE_STATIC_ENV = "NOVAPAW_CONSOLE_STATIC_DIR"
...
async def post_desktop_shutdown(
    x_novapaw_desktop_shutdown_token: str | None = Header(default=None),
):
    ...
    if (
        ...
        or x_novapaw_desktop_shutdown_token is None
        or not hmac.compare_digest(
            x_novapaw_desktop_shutdown_token,
            expected_token,
        )
    ):
...
from novapaw.browser.control_link import (  # noqa: E402
    register_builtin_control_links,
)

# pylint: disable-next=wrong-import-position,wrong-import-order
from novapaw.browser.control_link.chrome.ws_handler import (  # noqa: E402
    ws_router as browser_chrome_ws_router,
)

# pylint: disable-next=wrong-import-position,wrong-import-order
from novapaw.browser.control_link.chrome.observe import (  # noqa: E402
...
if os.path.isdir(_CONSOLE_STATIC_DIR):
    ...
    @app.get(
        "/{full_path:path}",
        name="novapaw_console_spa_catchall",
    )
