...
from novapaw.exceptions import (
    AppBaseException,
)
...
from ..mail.driver_config import (
    ...
    build_novapawmail_env as _build_novapawmail_env,
    generate_novapawmail_driver_card as _generate_novapawmail_driver_card,
    resolve_novapawmail_command as _resolve_novapawmail_command,
    sync_novapawmail_driver_card as _sync_novapawmail_driver_card,
)
...
async def list_agents(request: Request = None) -> AgentListResponse:
    ...
    for agent_id in ordered_agent_ids:
        ...
        try:
            ...
            if agent_config.backend == "novapaw":
...
async def update_backend_settings(
    ...
    def apply_settings(agent_config: AgentProfileConfig) -> None:
        if agent_config.backend == "novapaw":
            raise HTTPException(
                status_code=409,
                detail="NovaPaw models use the native model configuration",
            )
...
def _resolve_custom_workspace_dir(raw_workspace_dir: str) -> Path:
    ...
    relative value is anchored to the NovaPaw working directory rather
...
async def _require_novapawmail_driver_card(
    workspace_dir: Path,
    ...
    try:
        ...
        logger.warning(
            "Failed to prepare novapawmail driver during agent %s: %s",
    ...
    if not synced:
        raise HTTPException(
            ...
            detail=(
                ...
                "novapawmail driver card could not be generated. Check "
                "workspace permissions and retry."
            ),
        )


async def _rollback_novapawmail_update(
    agent_id: str,
    ...
    if config_restored:
        try:
            driver_restored = await run_sync_io(
                ...
                previous_config.backend or "novapaw",
                force_rewrite=previous_config.mail is not None,
            )
        except Exception:  # pylint: disable=broad-except
            driver_restored = False
            logger.exception(
                "Failed to restore novapawmail driver for agent %s",
                sanitize_log_value(agent_id),
            )
...
async def create_agent(
    ...
    active_model = (
        request.active_model if request.backend == "novapaw" else None
    )
    if request.backend == "novapaw" and (
    ...
    if request.mail is not None:
        await _require_novapawmail_driver_card(
            workspace_dir,
            request.mail,
            "creation",
        )
...
def _build_copied_agent_config(
    *,
    ...
    # Mail is only supported for the novapaw backend; silently drop it so
    # copies of legacy "third-party backend + mail" profiles stay valid
    # and no novapawmail driver card is generated for the new agent.
    if agent_config.backend != "novapaw":
...
async def update_agent(  # pylint: disable=too-many-statements
    ...
    effective_backend = (
        ...
        else getattr(existing_config_snap, "backend", "novapaw")
    ) or "novapaw"
    ...
    try:
        ...
        logger.warning(
            "Failed to synchronize novapawmail driver for agent %s",
            sanitize_log_value(agentId),
            exc_info=True,
        )
    ...
    if not driver_card_synced:
        ...
        (
            rollback_configured,
            rollback_driver,
        ) = await _rollback_novapawmail_update(
            agentId,
            previous_config,
            workspace_dir,
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=(
                "The novapawmail driver card could not be synchronized. "
                f"{rollback_detail} Check workspace permissions and retry."
            ),
        )

    if updated_config.mail is not None and updated_config.backend == "novapaw":
        await run_sync_io(
...
def _validate_mail_backend_compatibility(
    ...
    if mail is not None and (backend or "novapaw") != "novapaw":
        raise HTTPException(
            status_code=400,
            detail="Mail configuration is only supported for novapaw backend",
        )
...
def _validate_mail_config(mail: AgentMailConfig) -> None:
    ...
    if mail.is_new_account:
        ...
        # are no longer part of NovaPaw's dedicated-mailbox form.  A blank
