# -*- coding: utf-8 -*-
"""
MatrixChannel: NovaPaw BaseChannel implementation for Matrix (via matrix-nio).
...
logger = logging.getLogger("novapaw.channels.matrix")
...
# Known NovaPaw slash commands — used to decide whether to strip
# @mention prefix
...
class MatrixChannel(BaseChannel):
    """NovaPaw channel that connects to a Matrix homeserver via matrix-nio."""
    ...
    def from_config(
        ...
        return cls(
            ...
            device_name=raw.get("device_name", "novapaw-worker"),
    ...
    async def _on_room_event(
        ...
        # Skip history prepend for slash commands — NovaPaw's command parser
    ...
    # native content_parts → NovaPaw Content; same
    # vision_enabled guard as inbound media for image parts (§11).
    # ------------------------------------------------------------------

    # pylint: disable=too-many-return-statements
    def _build_content_part(self, p: dict[str, Any]) -> Any:
        """Convert a native content-part dict to a NovaPaw Content object."""
        t = p.get("type")
    ...
    def build_agent_request_from_native(self, native_payload: Any) -> Any:
        ...
        # in the same room share one session (NovaPaw keys session state on
