...
:class:`AgentBuilder` fully constructs a :class:`NovaPawAgent` for each
request.  It obtains tools from the per-workspace
:class:`NovaPawLocalWorkspace` (via ``list_tools``), the system prompt
...
class AgentBuilder:
    ...
    async def build_toolkit(
        ...
        *,
        ...
        :class:`NovaPawLocalWorkspace` via ``list_tools()``.
    ...
    async def build(  # pylint: disable=too-many-statements,too-many-branches
        ...
        """Construct a fully-wired :class:`NovaPawAgent` for one request.

        Integrates all per-workspace registries: NovaPawLocalWorkspace
        ...
        agent = NovaPawAgent(
            name=agent_config.name or "NovaPaw",
            model=model,
    ...
    def _build_context_config(agent_config: Any) -> Any:
        """Map NovaPaw's ``ContextCompactConfig`` to AS ``ContextConfig``."""
        ...
        try:
            ...
            # creates a replacement ToolResultBlock, and drops NovaPaw's
            ...
            return ContextConfig(
                ...
                # NovaPaw's visual compression owns the image budget and
    ...
    def _scroll_recall_runnable(agent_config: Any, governor: Any) -> bool:
        ...
        which requires BOTH the ``NOVAPAW_ALLOW_UNSANDBOXED_RECALL`` env var
    ...
    def _scroll_repl_runnable(agent_config: Any, governor: Any) -> bool:
        ...
        ``NOVAPAW_ALLOW_UNSANDBOXED_RECALL`` env var and
