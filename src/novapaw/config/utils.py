...
def _normalize_working_dir_bound_paths(data: object) -> object:
    """Normalize legacy ~/.novapaw-bound paths to current WORKING_DIR.

    This keeps NOVAPAW_WORKING_DIR effective even if user config files contain
    older hard-coded paths like "~/.novapaw/media" or
    "/Users/x/.novapaw/workspaces/...".
    Only rewrites known working-dir-bound keys.
    """
    legacy_root_tilde = "~/.novapaw"
...
def get_available_channels() -> Tuple[str, ...]:
    ...
    novapaw.channels), filtered by NOVAPAW_ENABLED_CHANNELS or
    NOVAPAW_DISABLED_CHANNELS when set.

    * NOVAPAW_ENABLED_CHANNELS — whitelist (only these channels are active).
    * NOVAPAW_DISABLED_CHANNELS — blacklist (all channels *except* these).
    * If both are set, NOVAPAW_ENABLED_CHANNELS takes precedence.
    * If neither is set, all discovered channels are returned.
    ...
    raw_enabled = EnvVarLoader.get_str("NOVAPAW_ENABLED_CHANNELS", "").strip()
    if raw_enabled:
...
def is_running_in_container() -> bool:
    ...
    Prefer env NOVAPAW_RUNNING_IN_CONTAINER (1/true/yes) at call time so
...
def is_novapaw_running() -> bool:
    """Check if NovaPaw is currently running by checking API availability.

    Returns:
        True if NovaPaw is running, False otherwise
    """
    try:
