...
class MatrixConfig(BaseChannelConfig):
    ...
    password: str = ""
    device_name: str = "novapaw-worker"
...
class VoiceChannelConfig(BaseChannelConfig):
    ...
    language: str = "en-US"
    welcome_greeting: str = "Hi! This is NovaPaw. How can I help you?"
...
class SIPChannelConfig(BaseChannelConfig):
    ...
    language: str = "zh-CN"
    welcome_greeting: str = "你好，我是NovaPaw"
...
class WeChatConfig(BaseChannelConfig):
    ...
    bot_token_file:         Path to persist/load the bot_token
                            (default ~/.novapaw/wechat_bot_token).
...
AGENT_MAIL_CREDENTIAL_REF = "mail/novapawmail"
    ...
    with _BUILTIN_TOOLS_LOCK:
        if _BUILTIN_TOOLS_CACHE is None:
            ...
            try:
                ...
                importlib.import_module("novapaw.agents.tools")
...
class ToolGuardConfig(BaseModel):
    ...
    auto_denied_rules: List[str] = Field(
        ...
        description=(
            ...
            "set env NOVAPAW_TOOL_GUARD_AUTO_DENIED_RULES=none."
        ),
    )
...
def build_fallback_agent_profile_config(
    ...
    Used by :func:`load_agent_config` and ``novapaw doctor fix``
...
def load_agent_config(  # pylint: disable=too-many-branches,too-many-statements
    ...
    with _agent_config_lock:
        ...
        # This keeps NOVAPAW_WORKING_DIR effective even if existing agent.json
        # contains older hard-coded paths like "~/.novapaw/media".
...
def migrate_legacy_config_to_multi_agent() -> bool:
    ...
    default_agent_config = AgentProfileConfig(
        id="default",
        name="Default Agent",
        description="Default NovaPaw agent",
    ...
    # When NOVAPAW_WORKING_DIR is customized, historical data may still exist
    # under "~/.novapaw".
    old_workspace = Path("~/.novapaw").expanduser().resolve()
