...
_REQUIREMENTS_METADATA_NAMESPACES = ("openclaw", "novapaw", "clawdbot")
...
def _extract_emoji_from_metadata(metadata: Any) -> str:
    """Extract emoji from metadata.novapaw.emoji."""
    if not isinstance(metadata, dict):
        return ""
    novapaw = metadata.get("novapaw")
    if isinstance(novapaw, dict):
        return str(novapaw.get("emoji", "") or "")
    return ""
...
def extract_zip_skills(data: bytes) -> tuple[Path, list[tuple[Path, str]]]:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="novapaw_skill_upload_"))
...
def staged_skill_dir(skill_name: str) -> Iterator[Path]:
    ...
    temp_root = Path(
        tempfile.mkdtemp(prefix=f"novapaw_skill_stage_{skill_name}_"),
    )
