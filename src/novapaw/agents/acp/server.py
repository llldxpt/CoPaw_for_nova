# -*- coding: utf-8 -*-
"""NovaPaw ACP Agent server.

Exposes NovaPaw as an ACP-compliant agent that external clients
...
ACP_ERROR_META_KEY = "novapaw.error"
ACP_AGENT_META_KEY = "novapaw.agent"
...
_GENERIC_PROMPT_ERROR = (
    "NovaPaw failed to process the request. Check server logs for details."
)
...
class NovaPawACPAgent(Agent):
    ...
    def _session_info(
        *,
        ...
        info: dict[str, Any] = {
            ...
            "mode": NovaPawACPAgent.MODE_DEFAULT,
        }
    ...
    async def _ensure_workspace(self) -> Any:
        ...
        logger.info(
            "NovaPaw ACP Agent workspace started: agent_id=%s workspace=%s",
            agent_id,
            workspace_dir,
        )
        return workspace
    ...
    async def _bridge_approval_requests(
        ...
        *,
        ...
        """Bridge NovaPaw ApprovalService waits to ACP permission prompts."""
    ...
    async def _request_approval_decision(
        ...
        """Ask the ACP client to approve/deny a NovaPaw pending approval."""
        from ...app.approvals import get_approval_service
    ...
    def _approval_options(pending: Any) -> list[PermissionOption]:
        """Build ACP permission options for a pending NovaPaw approval."""
        display = NovaPawACPAgent._approval_display(pending)
        if (
    ...
    def _session_meta(self) -> dict[str, Any] | None:
        """Return session ``_meta`` with the resolved NovaPaw agent id."""
        try:
            agent_id = self._resolve_agent_id()
        except Exception:
...
async def run_novapaw_agent(
    agent_id: str | None = None,
    workspace_dir: Path | None = None,
    local_diagnostics: bool = False,
) -> None:
    """Entry point: run NovaPaw as an ACP agent over stdio."""
    agent = NovaPawACPAgent(
        agent_id=agent_id,
