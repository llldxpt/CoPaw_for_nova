...
subclassing the agent, it is injected into :class:`NovaPawAgent` and drives the
two delegated hooks.
...
class ScrollContextManager:
    ...
    def _recall_page_stub(
        ...
        page = cls._block_metadata(block).get("novapaw_recall_page", {})
        next_cursor = (
    ...
    def _is_continuation_stub(msg: Any) -> bool:
        ...
        if not isinstance(metadata, dict):
            return False
        tag = metadata.get(NOVAPAW_MESSAGE_TAG_KEY)
    ...
    def _rebuild_context(
        ...
        placeholder = UserMsg(
            ...
            content=memory,
            metadata={
                NOVAPAW_MESSAGE_TAG_KEY: SCROLL_MEMORY_MESSAGE_TAG,
            },
        )
