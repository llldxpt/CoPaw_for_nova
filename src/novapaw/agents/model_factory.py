...
Example:
    >>> from novapaw.agents.model_factory import create_model_and_formatter
...
# once NovaPaw's minimum AgentScope version provides stable, request-unique
# identifiers; NovaPaw should then preserve the upstream identifiers.
...
_FORMATTER_SEEN_MEDIA_KEYS: ContextVar[set[str] | None] = ContextVar(
    "novapaw_formatter_seen_media_keys",
    default=None,
)
...
def _normalize_messages_for_formatter(
    ...
    if getattr(formatter_instance, "_novapaw_force_strip_media", False):
        supports_multimodal = False
    strip_audio = bool(
        getattr(formatter_instance, "_novapaw_force_strip_audio", False),
    )
...
def _create_file_block_support_formatter(
    ...
    class FileBlockSupportFormatter(base_formatter_class):
        ...
        async def format(self, msgs):
            ...
            self._novapaw_last_wire_media_count = 0
            self._novapaw_last_wire_audio_count = 0
            ...
            require_reasoning = getattr(
                self,
                "_novapaw_require_reasoning_content",
                False,
            )
            ...
            self._novapaw_last_wire_media_count = _count_wire_media_blocks(
                wire_messages,
            )
            self._novapaw_last_wire_audio_count = _count_wire_audio_blocks(
                wire_messages,
            )
...
def _bind_provider_id_to_model(
    ...
    bind_provider_id = getattr(model, "bind_novapaw_provider_id", None)
        ...
        if not global_model:
            raise ProviderError(
                message=(
                    ...
                    "Please configure a model using 'novapaw models config' "
...
def _create_formatter_instance(
    ...
    Returns:
        ...
        the model's native one, plus novapaw extensions for media
...
def _install_model_formatter(
    ...
    """Install and return the NovaPaw formatter for one model."""
