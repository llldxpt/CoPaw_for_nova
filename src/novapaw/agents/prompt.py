...
# Import Nova Studio prompt content
from .nova_studio_prompt import DEFAULT_NOVA_STUDIO_CONTENT
...
class PromptBuilder:
    ...
    def _load_file(self, filename: str, required: bool) -> bool:
        ...
        if not file_path.exists():
            if filename == "NOVA_STUDIO.md":
                content = DEFAULT_NOVA_STUDIO_CONTENT.strip()
            elif required:
                logger.warning(
...
def build_system_prompt_from_working_dir() -> str:
    ...
    WORKING_DIR (~/.novapaw by default). These files define the agent's behavior,
