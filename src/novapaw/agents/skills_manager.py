...
def sync_skills_from_active_to_customized(
    ...
    NOTE: Built-in skills are NOT synced to customized_skills to avoid duplicates.
...
def get_skill_version(skill_name: str) -> str | None:
    """
    Get the version of an installed skill from its SKILL.md metadata.
    ...
    try:
        ...
        if isinstance(metadata, dict):
            return metadata.get("version") or metadata.get("name")
    except Exception:
        pass
...
def list_installed_skills_with_versions() -> dict[str, str]:
    """
    List all installed skills with their versions.

    Returns:
        Dictionary mapping skill names to their versions.
    """
    active_skills = get_active_skills_dir()
    ...
    for d in active_skills.iterdir():
        if d.is_dir() and (d / "SKILL.md").exists():
            version = get_skill_version(d.name)
            result[d.name] = version if version else ""
...
def ensure_skills_initialized() -> None:
    ...
    Skills should be configured via `novapaw init` or
    `novapaw skills config`.
    """
    active_skills = get_active_skills_dir()
    ...
    if not active_skills.exists() or not available:
        logger.warning(
            ...
            "Run 'novapaw init' or 'novapaw skills config' "
            "to configure skills.",
        )
    else:
        logger.debug(
