...
INBOX_RESULT_HOOK_KEY = "novapaw_memory_result_hook"
_REME_SESSION_ID_HASH_PREFIX = "qpsid_sha256_"
_REQUIRED_REME_VERSION = "0.4.1.10"


class _ReMeContractError(RuntimeError):
    """Installed ReMe does not implement NovaPaw's pinned integration API."""
...
class ReMeLightMemoryManager(BaseMemoryManager):
    ...
    ReMe uses the NovaPaw workspace root as its vault.  Daily memory,
    ...
    async def start(self) -> None:
        ...
        try:
            await self._update_novapaw_model()
            ...
            logger.info(
                "ReMe starting without an active NovaPaw model for agent "
                "'%s': %s",
                self.agent_id,
                exc,
            )
    ...
    async def _update_novapaw_model(self) -> None:
        """Reuse NovaPaw's active model in ReMe's default LLM component."""
        if self._reme is None:
            return
    ...
    async def _run_reme_job(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            needs_llm: Refresh the injected NovaPaw model before running.
    ...
    def _install_reme_result_hook(self) -> None:
        """Expose NovaPaw inbox delivery to ReMe background steps."""
        if self._reme is None:
            return
