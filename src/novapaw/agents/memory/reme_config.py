# -*- coding: utf-8 -*-
"""Embedded ReMe application configuration for NovaPaw memory.

ReMe's standalone CLI normally loads YAML such as
``reme/config/default.yaml`` or ``reme/config/novapaw.yaml``.  NovaPaw embeds
...
from novapaw.config.config import AgentProfileConfig, EmbeddingModelConfig
...
def build_reme_app_config(
    *,
    ...
    """Build ReMe ``Application`` kwargs for embedded NovaPaw usage."""
...
def _base_config() -> dict[str, Any]:
    """Return the ReMe config shape used by NovaPaw."""
...
def _apply_embedding_config(
    ...
    """Map NovaPaw embedding config into ReMe component config."""
