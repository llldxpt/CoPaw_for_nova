...
from novapaw.tauri.env import (
    DESKTOP_APP_ENV,
    DESKTOP_CORS_ORIGINS_ENV,
    DESKTOP_READY_PREFIX,
    ensure_desktop_cors_origins,
)
from novapaw.tauri.sidecar_logging import install_sidecar_logging
...
def _bundled_python() -> str:
    ...
    python = (os.environ.get("NOVAPAW_DESKTOP_PY_RUNTIME") or "").strip()
...
def _child_env_with_plugin_site(env: "dict | None") -> "dict | None":
    ...
    """
    site_dir = (os.environ.get("NOVAPAW_PLUGIN_SITE") or "").strip()
    if not site_dir:
        return env
...
def _reexec_as_bundled_python(args: Sequence[str]) -> None:
    ...
    plugin-installed deps (``NOVAPAW_PLUGIN_SITE``) importable.
    """
    python = _bundled_python()
    if not python:
        print(
            "novapaw-backend is the desktop backend, not a Python "
...
def _install_subprocess_guard() -> None:
    ...
    * On Windows, suppress console windows for child processes that don't pass
    ...
    if getattr(subprocess.Popen, "_novapaw_guarded", False):
        return
    ...
    setattr(subprocess.Popen, "_novapaw_guarded", True)


def _ensure_novapaw_app_not_loaded() -> None:
    if "novapaw.app._app" in sys.modules:
        raise RuntimeError(
            "novapaw app imported before desktop CORS origins were set",
        )


def _sync_loaded_novapaw_constant_cors_origins() -> None:
    constant_module = sys.modules.get("novapaw.constant")
...
def _install_desktop_runtime() -> None:
    ...
    # from novapaw.constant.CORS_ORIGINS at import time.
    _ensure_novapaw_app_not_loaded()
    ensure_desktop_cors_origins()
    _sync_loaded_novapaw_constant_cors_origins()
...
def _run_backend_server(log_level: str) -> None:
    ...
    from novapaw.config.utils import write_last_api
    from novapaw.constant import LOG_LEVEL_ENV, WORKING_DIR
    from novapaw.utils.logging import (
        SuppressPathAccessLogFilter,
        setup_logger,
    )
    from novapaw.utils.port import get_stable_port, write_port_file
    ...
    os.environ.pop("NOVAPAW_RELOAD_MODE", None)
    ...
    from novapaw.tauri.backend_guard import reconcile_singleton_backend

    reconcile_singleton_backend(WORKING_DIR)
    if normalized_log_level in ("debug", "trace"):
        from novapaw.cli.main import log_init_timings
...
def main() -> None:
    ...
    if not config_path.exists():
        from novapaw.cli.init_cmd import init_cmd
