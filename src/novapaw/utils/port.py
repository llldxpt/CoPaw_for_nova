...
NOVAPAW_DESKTOP_PORT environment variable.
"""
...
from ..constant import NOVAPAW_DESKTOP_PORT
...
def get_stable_port(
    ...
    If NOVAPAW_DESKTOP_PORT environment variable is set, it takes priority
    and the port is fixed (not persisted).
    """
    # Check for user-configured port via environment variable
    if NOVAPAW_DESKTOP_PORT:
        try:
            port = int(NOVAPAW_DESKTOP_PORT)
        except (TypeError, ValueError):
            logger.warning(
                "NOVAPAW_DESKTOP_PORT=%r is not a valid "
                "integer, falling back to auto-assign",
                NOVAPAW_DESKTOP_PORT,
            )
            port = None
        if port is not None:
            if not 1024 <= port <= 65535:
                logger.warning(
                    "NOVAPAW_DESKTOP_PORT=%d out of range "
                ...
                if sock:
                    logger.info(
                        "Using NOVAPAW_DESKTOP_PORT: %d",
                        port,
                    )
                    return port, sock
                logger.warning(
                    "NOVAPAW_DESKTOP_PORT=%d is "
